...
public class DepthMapView extends BaseTextureView {
  ...
  private void init(AttributeSet attrs) {
    ...
    if (typedArray != null) {
      try {
        ...
        mDotRadius = typedArray.getDimensionPixelSize(R.styleable.DepthMapView_dot_radius,
            ResourceUtil.dp2px(getContext(), mDotRadius));
        mCircleRadius = typedArray.getDimensionPixelSize(R.styleable.DepthMapView_circle_radius,
            ResourceUtil.dp2px(getContext(), mCircleRadius));
        mBackgroundColor = typedArray.getColor(R.styleable.DepthMapView_background_color,
            ResourceUtil.getColor(getContext(), R.color.white));
        mBuyLinePaint.setStrokeWidth(
            typedArray.getDimensionPixelSize(R.styleable.DepthMapView_line_width,
                ResourceUtil.dp2px(getContext(), 1.5f)));
        mSellLinePaint.setStrokeWidth(
            typedArray.getDimensionPixelSize(R.styleable.DepthMapView_line_width,
                ResourceUtil.dp2px(getContext(), 1.5f)));
        mTextPaint.setColor(typedArray.getColor(R.styleable.DepthMapView_text_color,
            ResourceUtil.getColor(getContext(), R.color.depth_text_color)));
        mTextPaint.setTextSize(typedArray.getDimension(R.styleable.DepthMapView_text_size,
            ResourceUtil.getDimension(getContext(), R.dimen.text_10sp)));
        mBuyLinePaint.setColor(typedArray.getColor(R.styleable.DepthMapView_buy_line_color,
            ResourceUtil.getColor(getContext(), R.color.color_00a546)));
        mSellLinePaint.setColor(typedArray.getColor(R.styleable.DepthMapView_sell_line_color,
            ResourceUtil.getColor(getContext(), R.color.color_a50000)));
        mBuyPathPaint.setColor(typedArray.getColor(R.styleable.DepthMapView_buy_path_color,
            ResourceUtil.getColor(getContext(), R.color.color_3300a546)));
        mSellPathPaint.setColor(typedArray.getColor(R.styleable.DepthMapView_sell_path_color,
            ResourceUtil.getColor(getContext(), R.color.color_33a50000)));
        mSelectorBackgroundPaint.setColor(
            typedArray.getColor(R.styleable.DepthMapView_selector_background_color,
                ResourceUtil.getColor(getContext(), R.color.depth_selector)));
      } finally {
        typedArray.recycle();
      }
    }
  ...
  @Override void onAsyncDraw(Canvas canvas) {
  ...
  private void drawBuy(Canvas canvas) {
    ...
    for (int i = 0; i < mBuyData.size(); i++) {
      ...
      if (i >= 1) {
        canvas.drawLine(mGridWidth * (i - 1), getY(mBuyData.get(i - 1).getVolume()), mGridWidth * i,
            y, mBuyLinePaint);
      }
      if (i != mBuyData.size() - 1) {
        mBuyPath.quadTo(mGridWidth * i, y, mGridWidth * (i + 1),
            getY(mBuyData.get(i + 1).getVolume()));
      }
  ...
  private void drawSelectorView(Canvas canvas, int position) {
    ...
    String volume = getContext().getString(R.string.trust_quantity) + ": " + getVolumeValue(
        mMapX.get(position).getVolume());
    String price = getContext().getString(R.string.trust_price) + ": " + getValue(
        mMapX.get(position).getPrice());
    ...
    canvas.drawRoundRect(
        new RectF(mDrawWidth - width / 2 - padding, 0, mDrawWidth + width / 2 + padding * 2,
            textHeight * 2 + padding * 2), 10, 10, mSelectorBackgroundPaint);
    canvas.drawText(getContext().getString(R.string.trust_quantity) + ": ",
        mDrawWidth - width / 2 + padding + mTextPaint.measureText(
            getContext().getString(R.string.trust_quantity)), textHeight + 2, mTextPaint);
    canvas.drawText(getVolumeValue(mMapX.get(position).getVolume()),
        mDrawWidth + width / 2 + padding, textHeight + 2, mTextPaint);
    canvas.drawText(getContext().getString(R.string.trust_price) + ": ",
        mDrawWidth - width / 2 + padding + mTextPaint.measureText(
            getContext().getString(R.string.trust_price)), textHeight * 2 + padding, mTextPaint);
    canvas.drawText(getValue(mMapX.get(position).getPrice()), mDrawWidth + width / 2 + padding,
        textHeight * 2 + padding, mTextPaint);
  }
