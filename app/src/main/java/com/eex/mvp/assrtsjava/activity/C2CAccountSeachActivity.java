...
import com.eex.mvp.assrtsjava.adapter.C2CAccountSeachAdapter;
 ...
 * @ClassName: C2CAccountSeachActivity
 * @Description: 法币搜索
 * @Author: hcj
 ...
 * @UpdateRemark: 更新说明
 * @Version: 1.0
 * C2CAccountSeachAdapter
 */

public class C2CAccountSeachActivity extends BaseActivity implements BaseQuickAdapter.OnItemClickListener {
    ...
    private List<CtcaccountList> mNewList = new ArrayList<CtcaccountList>();
    private C2CAccountSeachAdapter adapter;
    ...
    protected void setView() {
        adapter = new C2CAccountSeachAdapter(mNewList);
    ...
    public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
        ...
        intent.putExtra("imgurl", mNewList.get(position).getImgUrl());
        intent.putExtra("totalAssets", mNewList.get(position).getTotalAssets().setScale(8, BigDecimal.ROUND_DOWN).stripTrailingZeros().toPlainString());
        intent.putExtra("usableMoney", mNewList.get(position).getUsableMoney().setScale(8, BigDecimal.ROUND_DOWN).stripTrailingZeros().toPlainString());
        ...
        intent.setClass(getActivity(), C2CAccountDetailsActivity.class);
