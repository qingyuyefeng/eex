 ...
 * @ProjectName: EEX
 * @Package: com.eex.mvp.assrtsjava.activity
 * @ClassName: FuturesChargeMoneActivity
 * @Description: java类作用描述
 * @Author: hcj
 * @CreateDate: 2020/1/8 19:54
 * @UpdateUser: 更新者
 * @UpdateDate: 2020/1/8 19:54
 * @UpdateRemark: 更新说明
 * @Version: 1.0
 */

public class FuturesChargeMoneActivity extends BaseActivity {


  @BindView(R.id.comtitlebar) NewComTitleBar comtitlebar;
  @BindView(R.id.chargemone_name) TextView chargemoneName;
  @BindView(R.id.chargemone_line) LinearLayout chargemoneLine;
  @BindView(R.id.Capital_account) TextView CapitalAccount;
  @BindView(R.id.Capital_image) ImageView CapitalImage;
  @BindView(R.id.Capital_c2c) TextView CapitalC2c;
  @BindView(R.id.Capital_please) EditText CapitalPlease;
  @BindView(R.id.Capital_edtoMuber) EditText CapitalEdtoMuber;
  @BindView(R.id.Capital_name) TextView CapitalName;
  @BindView(R.id.Capital_all) TextView CapitalAll;
  @BindView(R.id.Capital_money) Button CapitalMoney;
   ...
   * 全部数据
   */
  private ArrayList<Assets> data = new ArrayList();
  /**
   * c2c余额
   */
  ...
  @Override protected int getLayoutId() {
    return R.layout.re_actvity_futures_choice;
  }
    ...
    RxExtensionKt.filterResult(ApiFactory.getInstance()
        ...
        .subscribe(ctcaccountData -> {
          ...
          if (ctcaccountData.getData() != null) {
            data.clear();
            data.addAll(ctcaccountData.getData());
    ...
    switch (view.getId()) {
      ...
      case R.id.Capital_image:
        CapitalImage.setEnabled(false);
          if (type.equals("1")) {
            type = "2";
            CapitalAccount.setText(mContext.getResources().getString(R.string.Contract_account));
            CapitalC2c.setText(mContext.getResources().getString(R.string.Capital_account));
            CapitalEdtoMuber.setText(C2CMoney.setScale(8, BigDecimal.ROUND_DOWN).stripTrailingZeros().toPlainString());
          } else {
            type = "1";
            CapitalAccount.setText(mContext.getResources().getString(R.string.Capital_account));
            CapitalC2c.setText(mContext.getResources().getString(R.string.Contract_account));
            CapitalEdtoMuber.setText(ZijinMoney.setScale(8, BigDecimal.ROUND_DOWN).stripTrailingZeros().toPlainString());
          }
...
//            CapitalPlease.setText(UsableMoney);
          } else {
...
//            CapitalPlease.setText(UsableMoney);
  ...
  private void putData(String type) {
    ...
    hashMap.put("money", CapitalPlease.getText().toString().trim());
    RxExtensionKt.filterResult(ApiFactory.getInstance()
        .gettransferAssets(kv.decodeString("tokenId"), hashMap))
        .compose(RxSchedulers.io_main())
        .subscribe(data -> {
