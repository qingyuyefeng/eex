 ...
 * @Version: 1.0
 */
public class MyUpFrament extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener {
    ...
    private void setViewList(ArrayList<AdvertisingUser> list) {
        adapter = new MyAdverAdapter(R.layout.item_my_adver, list);
        ...
        adapter.setOnItemChildClickListener(new BaseQuickAdapter.OnItemChildClickListener() {
            ...
            public void onItemChildClick(BaseQuickAdapter adapter, View view, int position) {
                ...
                switch (view.getId()) {
                    //编辑
                    case R.id.LLEditors:
                        Toast.makeText(getActivity(), "请先下架广告", Toast.LENGTH_SHORT).show();

                        break;
                    //下架
                    case R.id.LLLower:
                        View view1 = getActivity().getLayoutInflater().inflate(R.layout.dialog_me_down, null);
                        ...
                        btn_yes.setOnClickListener(new View.OnClickListener() {
                            @Override
                            public void onClick(View v) {
                                LowerBaner(list.get(position).getId(), position);
                                mMyDialog.dismiss();
                            }
    ...
    @SuppressLint("CheckResult")
    private void LowerBaner(String id, int position) {

        HashMap<String, String> hashMap = new HashMap<>();
        hashMap.put("id", id);

        ApiFactory.getInstance()
                .obtainedAdvertising(kv.decodeString("tokenId"), hashMap)
                .compose(RxSchedulers.io_main())
                .subscribe(data -> {
                    SwipeUtil.loadCompleted(swipeRefresh);
                    if (data.getCode() == 200) {
