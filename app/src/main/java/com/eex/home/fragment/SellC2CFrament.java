...
import com.eex.home.adapter.SellC2CAdapter;
...
public class SellC2CFrament extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener {

  private static final String TAG = "SellC2CFrament";
  ...
  public static SellC2CAdapter ListAdapter;
  /**
   * 买入
   */
  public static ArrayList<Advertisingvo> listC2Sell = new ArrayList<>();
    ...
    ApiFactory.getInstance()
        ...
        .subscribe(arrayListData -> {
          ...
          if (arrayListData == null) {
            ...
            list = new ArrayList<CoinfigList>();
            list.clear();
    ...
    ApiFactory.getInstance()
        ...
        .subscribe(data -> {
          ...
          if (data.getData().getPageNo() > 1) {
            ...
            //                        //存储Advertisingvo数据
            //                        MMKV kv = MMKV.mmkvWithID("Advertisingvo", MMKV.MULTI_PROCESS_MODE);
            //
            //                        for (int i = 0; i < advertisingvos.size(); i++) {
            //
            //                            kv.encode("maxTradeNum", (Parcelable) advertisingvos.get(i).getMaxTradeNum());
            //                            kv.encode("minTradeNum", (Parcelable) advertisingvos.get(i).getMinTradeNum());
            //                            kv.encode("premium", (Parcelable) advertisingvos.get(i).getPremium());
            //                            kv.encode("tradePrice", (Parcelable) advertisingvos.get(i).getTradePrice());
            //                            kv.encode("quantity", (Parcelable) advertisingvos.get(i).getQuantity());
            //                            kv.encode("tradeCount", advertisingvos.get(i).getTradeCount().MIN_VALUE);
            //                            kv.encode("tradeOKCount", advertisingvos.get(i).getTradeOKCount().MIN_VALUE);
            //                            kv.encode("merchName", advertisingvos.get(i).getMerchName());
            //                            kv.encode("merchantStatus", advertisingvos.get(i).getMerchantStatus().MIN_VALUE);
            //                            kv.encode("tradeCoin", advertisingvos.get(i).getTradeCoin());
            //                            kv.encode("id", advertisingvos.get(i).getId());
            //                            kv.encode("tradeNum", (Parcelable) advertisingvos.get(i).getTradeNum());
            //                        }
            ...
            if (data.getData().getResultList() != null) {
              listC2Sell.addAll(data.getData().getResultList());
              pageNo = 2;
            }

            Collections.sort(listC2Sell, new Comparator<Advertisingvo>() {
              @Override
            ...
            Collections.reverse(listC2Sell);
            setListView(listC2Sell);
          }
        }, throwable -> {
          dialog.dismiss();
          SwipeUtil.loadCompleted(swipeRefresh);
  ...
  private void setListView(final List<Advertisingvo> listC2Sell) {

    ListAdapter = new SellC2CAdapter(R.layout.item_sellframen, listC2Sell);
          ...
          switch (view.getId()) {
            case R.id.btn_sell:
              if (kv.decodeString("tokenId") == null) {
                ...
                view = getActivity().getLayoutInflater().inflate(R.layout.dialog_c2cbuysell, null);
              ...
              if (data.getData().getLevel().equals(3)) {
                ...
                if (data.getData().getLevel().equals(2)) {
                  if (kv.decodeString("phone") != null && !kv.decodeString("phone").equals("")) {
                    ...
                    view =
                        getActivity().getLayoutInflater().inflate(R.layout.dialog_c2cbuysell, null);
                    Tltle = (TextView) view.findViewById(R.id.tltle);
                    btnDimss = (Button) view.findViewById(R.id.btn_dimss);
                    btnSell = (Button) view.findViewById(R.id.btn_sell);
                    ...
                    btnSell.setOnClickListener(new View.OnClickListener() {
                      ...
                      public void onClick(View v) {
                        ...
                        intent.setClass(getContext(), PhoneNameActivity.class);
                        startActivity(intent);
                        mMyDialog.dismiss();
                      }
                    });
                    ...
                    public void onClick(View v) {
                      if (kv.decodeString("phone") != null && !kv.decodeString("phone")
                        ...
                        view = getActivity().getLayoutInflater()
                            .inflate(R.layout.dialog_c2cbuysell, null);
                        Tltle = (TextView) view.findViewById(R.id.tltle);
                        btnDimss = (Button) view.findViewById(R.id.btn_dimss);
                        btnSell = (Button) view.findViewById(R.id.btn_sell);
                        mMyDialog = new MyDialog(getActivity(), 0, 0, view, R.style.DialogTheme);
    ...
    ApiFactory.getInstance()
        ...
        .subscribe(data -> {
          ...
          if (data.getData() != null && data.getData().size() != 0) {
            //发布广告
            intent.putExtra("id", listC2Sell.get(position).getId());
            intent.setClass(getActivity(), SellDetelisActivity.class);
            startActivity(intent);
          } else {
