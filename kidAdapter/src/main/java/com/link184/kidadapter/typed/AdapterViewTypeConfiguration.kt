...
class AdapterViewTypeConfiguration {
  ...
  var layoutResId: Int = -1
    private set
  val resIdMap = hashMapOf<String, @LayoutRes Int>()
  internal var bindHolderIndexed: View.(Any, Int) -> Unit = { _, _ -> }
  internal var bindHolderLocation: View.(Any, Int, Int) -> Unit = { _, _, _ -> }
  ...
  @ConfigurationDsl
  fun withLayoutResIdByTag(@LayoutRes layoutResId: Int, tag: String) {
    this.resIdMap[tag] = layoutResId
  }
  ...
  @BindDsl
  fun <T> bindIndexed(block: View.(T, Int, Int) -> Unit) {
    bindHolderLocation = (block as View.(Any, Int, Int) -> Unit)
  }
  ...
  internal fun addAllToInternalItems(
    index: Int,
  ...
  internal fun swapInternalItems(
    firstIndex: Int,
    secondIndex: Int
  ) {
