...
class TypedKidAdapterConfiguration {
  ...
  fun withViewType(
    tag: String? = null,
    block: AdapterViewTypeConfiguration.() -> Unit
  ) {
    val fromPosition =
      viewTypes.fold(
          0
      ) { acc, adapterViewType -> acc + adapterViewType.configuration.getInternalItems().size }
  ...
  internal fun getAllItems(): MutableList<Any> {
    val alignedItems = viewTypes
        .map {
          it.configuration.getInternalItems()
              .toMutableList()
        }
  ...
  internal fun invalidateItems() {
    ...
    viewTypes.forEach { adapterViewType ->
      ...
      adapterViewType.positionRange = fromPosition until toPosition
    }
  }
