...
abstract class BaseAdapter<T, H : BaseViewHolder<T>>(protected var itemList: KidList<T>) :
    RecyclerView.Adapter<H>(),
    LoadMoreInteractor {
  constructor(itemList: MutableList<T>) : this(KidList(itemList))

  private val loadMoreHelper = LoadMoreHelper()
  ...
  final override fun onBindViewHolder(
    ...
    if (position == itemList.size - 1 && itemList.size > 1) {
  ...
  override fun setLoadMoreEnable(enable: Boolean) = loadMoreHelper.setLoadMoreEnable(enable)

  override fun setLoadMoreListener(callbk: () -> Unit) = loadMoreHelper.setLoadMoreListener(callbk)
}