...
public class DataTools {
  ...
  float[] calculate(List<? extends KLineEntity> dataList, float bollP, int bollN,
    ...
    for (int i = 0; i < size; i++) {
      ...
      float closePrice = point.getClosingPrice();
      ...
      points[indexInterval * i + Constants.INDEX_HIGH] = point.getTopPrice();
      points[indexInterval * i + Constants.INDEX_OPEN] = point.getOpeningPrice();
      points[indexInterval * i + Constants.INDEX_LOW] = point.getFloorPrice();
      points[indexInterval * i + Constants.INDEX_CLOSE] = point.getClosingPrice();
      points[indexInterval * i + Constants.INDEX_VOL] = point.getDealQuantity();
      ...
      if (i == priceMaOne - 1) {
        ...
        maSum1 -= dataList.get((int) (i - priceMaOne)).getClosingPrice();
        point.setMaOne((float) (maSum1 / priceMaOne));
      } else {
      ...
      if (i == priceMaTwo - 1) {
        ...
        maSum2 -= dataList.get((int) (i - priceMaTwo)).getClosingPrice();
        point.setMaTwo((float) (maSum2 / priceMaTwo));
      } else {
      ...
      if (i == priceMaThree - 1) {
        ...
        maSum3 -= dataList.get((int) (i - priceMaThree)).getClosingPrice();
        point.setMaThree((float) (maSum3 / priceMaThree));
      } else {
      ...
      //vol ma计算
      volumeMaOne += point.getDealQuantity();
      volumeMaTwo += point.getDealQuantity();
      float ma;
      if (i == maOne - 1) {
        ma = (float) (volumeMaOne / maOne);
      } else if (i > maOne - 1) {
        volumeMaOne -= dataList.get((int) (i - maOne)).getDealQuantity();
        ma = (float) (volumeMaOne / maOne);
      } else {
        ma = Float.MIN_VALUE;
      }
      ...
      if (i == maTwo - 1) {
        ...
        volumeMaTwo -= dataList.get((int) (i - maTwo)).getDealQuantity();
        ma = (float) (volumeMaTwo / maTwo);
      } else {
        ma = Float.MIN_VALUE;
      }
    ...
    for (int i = start; i < end; i++) {
      ...
      if (i == n) {
        ...
        for (int k = 1; k <= n; k++) {
          ...
          double close = kLineEntity.getClosingPrice();
          double lastClose = klineInfos.get(k - 1).getClosingPrice();
        ...
        double close = klineInfos.get(i).getClosingPrice();
        double lastClose = klineInfos.get(i - 1).getClosingPrice();
  ...
  private void calcKdj(List<? extends KLineEntity> dataList, int kdjDay, int i, KLineEntity point,
      float closePrice) {
    float k, d;
    if (i < kdjDay - 1 || 0 == i) {
      ...
      for (int index = startIndex; index <= i; index++) {
        maxRsi = Math.max(maxRsi, dataList.get(index).getTopPrice());
        minRsi = Math.min(minRsi, dataList.get(index).getFloorPrice());
      }
      float rsv;
      try {
  ...
  private float calculateEma(List<? extends KLineEntity> list, int n, int index, float preEma) {
    try {
      ...
      if (index + 1 < n) {
        ...
        for (int i = 0; i < n; i++) {
          y += list.get(i).getClosingPrice();
        }
        return y / n;
      } else {
        return (preEma * (n - 1) + list.get(index).getClosingPrice() * 2) / (n + 1);
      }
    } catch (Exception e) {
      e.printStackTrace();
      return 0;
  ...
  public float calcWr(List<? extends KLineEntity> dataDiction, int nIndex, int n) {
    ...
    if (valueSpan > 0) {
      ...
      return 100 * (highInHighsValue - kLineData.getClosingPrice()) / valueSpan;
    } else {
      return 0;
    }
  }
  ...
  public float getMin(List<? extends KLineEntity> valuesArray, int fromIndex, int nCount) {
    ...
    if (fromIndex >= endIndex) {
      for (int itemIndex = fromIndex + 1; itemIndex > endIndex; itemIndex--) {
        ...
        float lowPrice = klineData.getFloorPrice();
  ...
  public float getMax(List<? extends KLineEntity> valuesArray,
    ...
    if (fromIndex >= endIndex) {
      for (int itemIndex = fromIndex + 1; itemIndex > endIndex; itemIndex--) {
        KLineEntity klineData = valuesArray.get(itemIndex - 1);
        float highPrice = klineData.getTopPrice();
  ...
  private float calculateBoll(List<? extends KLineEntity> payloads, int position, int maN) {
    ...
    for (int i = position; i >= position - maN + 1; i--) {
      sum = (sum + payloads.get(i).getClosingPrice());
    }
    return sum / maN;
  }
  ...
  private float STD(List<? extends KLineEntity> payloads, int positon, int maN) {
    ...
    for (int i = positon; i >= positon - maN + 1; i--) {
      sum += payloads.get(i).getClosingPrice();
    }
    float avg = sum / maN;
    for (int i = positon; i >= positon - maN + 1; i--) {
      std += (payloads.get(i).getClosingPrice() - avg) * (payloads.get(i).getClosingPrice() - avg);
    }
